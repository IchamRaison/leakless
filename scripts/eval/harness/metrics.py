...
def pr_auc(y: np.ndarray, s: np.ndarray) -> float:
    ...
    y_sorted = y[order]
    # Average precision : un seuil par score distinct, jamais un ordre entre ex aequo.
    ends = np.r_[np.flatnonzero(np.diff(s[order])), len(s) - 1]
    tp = np.cumsum(y_sorted)[ends]
    precision = tp / (ends + 1)
    return float(np.sum(np.diff(np.r_[0, tp]) * precision) / np.sum(y))
