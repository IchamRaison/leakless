...
Vérifier les changements A/C dans `.venv-repro`, créer un nouveau cache commun avec égalité mesures/textes WAV–TimeF sur 806 entrées, puis deux références A/C distinctes (mêmes poids V1, interdites comme warm-start CV). Exécuter pour chacune le gate 209 complet et son reload neuf. Ensuite préinscrire A/C, la recette commune et les folds exacts, six fits comparatifs puis modèle final frais / seuil validation / confirmation externe. Ne pas relancer la sonde, l'ancien gate ou l'audit de recouvrement déjà terminés. Le benchmark final à trois approches et les données d'événements/contexte restent une étape distincte non validée.
