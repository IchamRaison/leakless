...
**Déploiement A0 demandé par Icham et vérifié sur `ich@195.242.28.46`.** Le service V1 lancé dans le tour précédent, et non préexistant sur cette H100, a été arrêté et désactivé avant remplacement. Le poids temporel A/fold0 de la campagne `c13fd47` (`temporal.pt` SHA `932119cadd23f48945c09699c106278378c12aa092830094c786c4a7c10b02f6`) a été transféré depuis la H100 source et assemblé avec la base Qwen déjà vérifiée. Le service persistant écoute maintenant sur `0.0.0.0:8000` et son accès public `http://195.242.28.46:8000/health` a été vérifié depuis une autre machine. Routes actives : `GET /health`, `POST /a0/score`, `POST /a0/predict`, `/docs`. Smoke test réel : modèle déclaré `pipe-qwen3.5-4b-v2-A-fold0-c13fd47`, preprocessing V2 canonique, score fini `0.4379916632716556` sur la fixture de reload disponible, prédiction générée `no_leak`, audio invalide refusé HTTP 422 ; service `active/enabled`, environ 9,2 Gio GPU. A0 reste un modèle de fold de développement, sans seuil calibré ni statut de modèle final.
...
## Campagne A/C préinscrite avant tout fit

Assemblage `c13fd475a671cf7c7ef37987a90914049b9a454c` publié et transféré, **156 tests runtime sans skip** (111 TSLM +45 évaluation). Préinscription réelle terminée normalement, aucun fit : `/home/hicham/pipe-v0/artifacts/v2-campaign-c13fd47-001/preregistration.json`, SHA **`5c5e0b4bbed6b973853e53b658350815b2d963547b68d0a08a9ade50c28b1359`**. Reçu et logs publiés `550c7a6`, `docs/evidence/tslm-v2/campaign-c13fd47/`.

Les 598 WAV ont été revérifiés avant écriture du reçu ; mêmes trois folds diagnostiques, caches A/C806, base Qwen et deux gates PASS. Recette commune quatre époques, seed20260912, batch effectif8/microbatch1 ; aucun réglage val/test. Six fits TSLM +12 fits de comparaison C1 prévus ; les six anciennes sondes diagnostiques sont déclarées séparément. La règle de classement préinscrite ne comporte pas de gain minimal numérique : examiner les trois folds, AUC clip/groupe et FN/FP au seuil diagnostique0,5 face au C1 retenu. **Pause explicite après `select`, avant `fit-final`** ; premier du classement ne signifie pas progrès suffisant. Ne pas inventer de veto numérique après lecture, ni prolonger la recherche.
