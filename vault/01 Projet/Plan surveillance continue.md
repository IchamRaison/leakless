...
**Planification complémentaire du 13 septembre :** [[Plan C1 - suivi temporel et notifications]] décrit la variante acoustique C1 → suivi simple ou LSTM causal → restitution/notification. M1 puis M2 sous critères ; aucune implémentation ni réaffectation de réserve autorisée par cette demande de plan. Endpoint placeholder d'Icham sur `195.242.28.46` à préserver pour la V1 de repli, disponibilité et déploiement réel non vérifiés. Le plan ci-dessous conserve les jalons historiques du TSLM et le cap produit ; la variante n'est pas présentée comme un TSLM entraîné par simple ajout d'un LLM de notification.
...
**Complément utilisateur du 13 septembre :** le rôle spécifique du TSLM est à démontrer dans l'explication de l'évolution des événements et l'investigation interactive, éventuellement avec plusieurs signaux/contextes effectivement disponibles. Comparaison finale obligatoire : classifieur + DSP + gabarit, classifieur + mesures/contexte + Qwen, TSLM + séries/contexte. Aucune supériorité présumée ; la détection binaire ou une bande dominante ne justifient pas seules un modèle de langage. [[Plan V2 - fiabilité et parité des scores#7. Objectif final — démontrer la valeur du TSLM]] définit les preuves attendues et distingue ce cap de la campagne actuelle sur extraits.
...
Deux cadences proposées : score de classe régulier, texte lors de l'ouverture d'un événement ou sur demande. Dans la brique TSLM actuelle, le score vient du TSLM adapté au signal. Cela ne préjuge pas de l'architecture produit retenue : le benchmark final doit aussi tester explicitement le classifieur accompagné de Qwen et le classifieur accompagné d'un gabarit. La durée, les horaires et les comptes de fenêtres viennent du logiciel ; une description d'un extrait par le TSLM ne doit pas être présentée comme une analyse de plusieurs minutes.
