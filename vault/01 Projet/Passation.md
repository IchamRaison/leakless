...
**Matériel envisagé par Icham : quatre H100 sur quatre machines séparées.** Nouvel accès vérifié : `ich@195.242.28.46`, hôte `computeinstance-e00fgs9mdc3y0zkpk7`, H100 80GB HBM3 (81 559 Mio), pilote 580.173.02 ; GPU à 0 Mio / 0 % au contrôle. Python 3.12.3 et environ 1,2 To libres sur `/` ; environnement ML et données pas encore préparés ni vérifiés. Avec `hicham@89.169.123.193` vérifié lors de l'échange précédent, deux accès H100 ont maintenant réussi, pas quatre machines prêtes à entraîner.
...
**D2 implémenté, testé et préinscrit avant fit :** code `303609e`, 11 tests ciblés puis 214 tests de régression runtime sans skip (163 TSLM + 51 évaluation). Sources, caches et références vérifiés ; les 598 séries courantes sont identiques octet par octet aux entrées des anciennes sondes. Préinscription SHA `67ed6bd5…` et logs publiés à `3505928`, dossier `docs/evidence/tslm-v2/causal-d2-001/`. Prochaine action : exécuter une fois les trois fits HGB/200 itérations, puis vérifier leurs scores et comparaisons réservées. Aucun fit réel D2 encore ; seuls les tests synthétiques ont appris. [[Diagnostic causal - exécution#D2 — sonde non linéaire, protocole avant fit]]. CPU uniquement pour ce petit contrôle indépendant ; apprentissages TSLM toujours sur H100.
