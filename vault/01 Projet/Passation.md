...
**Deux H100 et deux runtimes vérifiés.** `hicham@89.169.123.193` conserve données/poids et exécute D3 ; `ich@195.242.28.46`, H100 80GB HBM3, possède désormais `/home/ich/pipe-v0/.venv-repro`, reconstruit avec uv et le lockfile existants sur le snapshot `a18b22e`. Installation de 132 paquets, imports, calcul/backward CUDA et 214 tests réussis sans skip. Preuves code `docs/evidence/runtime-h100-2-001/`. Poids/données non copiés sur la seconde à ce jalon : runtime opérationnel, pas encore un fit Qwen vérifié. Prochaine préparation : copies ciblées vérifiées avant de répartir des essais indépendants justifiés ; aucun essai ajouté ni mémoire mutualisée.
...
**D3 implémenté et préinscrit, avant fit :** code `c08ff78`, revue indépendante favorable ; huit tests ciblés puis 222 tests runtime sans skip. Un seul A neuf sur les 32 témoins D0, maximum 1 000 pas, loss/scoring/Qwen BF16 inchangés ; arrêt seulement si NLL binaire <0,1 et 32/32 décisions correctes. Préinscription SHA `42920f6d5446f34f7419ea2a1c5c1f47067e2a915bb2361fa0bdee4097e93a35`, dossier `/home/hicham/pipe-v0/artifacts/causal-d3-c08ff78-001`. Prochaine action : publier les preuves puis lancer `train`, suivre son handle et exécuter `reload` dans un processus neuf. [[Diagnostic causal - exécution#D3 — vérifications avant fit]]. Aucun résultat D3 à ce jalon ; mémorisation ≠ généralisation.
