...
**V2 — 13 septembre : entraînements repris, objectif final TSLM publié.** [[Plan V2 - fiabilité et parité des scores#7. Objectif final — démontrer la valeur du TSLM]] : comprendre/investiguer les événements, comparer gabarit, Qwen sur mesures et TSLM sur séries ; aucune capacité présumée acquise. [[V2 ML - exécution]] : deux gates A/C/reloads PASS, deltas nuls ; code final `c13fd47`, 156 tests runtime sans skip et campagne préinscrite SHA `5c5e0b4b…` avant fit. **A/fold0 actif (`10635`), file C0→C1→C2 active (`77795`) sur la H100** ; premières pertes/gradients A valides. C1 a terminé ses12 fits, `C=0.01` retenu sur train. Aucun résultat TSLM complet, seuil final ou score externe encore disponible. Prochaine action : suivre ces jobs sans redémarrage, lancer A1→A2 après A0, puis classer et examiner les six résultats avant tout refit. Le classement seul n'autorise pas automatiquement la suite. **Checkout actif `/home/animus/ehl-hackathon-zurich-v2-recovery`** ; ancien Git endommagé conservé, ne pas y commiter. Le benchmark final à trois approches reste distinct de cette campagne sur extraits.
