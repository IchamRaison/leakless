...
**Plan consolidé proposé, pas démarré :** [[Plan surveillance continue]] §4 C1–C4 et §5 sépare la livraison ML prioritaire (score vérifié → V1 train/validation → gel/reload → T0 à Nevil → stress optionnels) de la boucle produit menée en parallèle (flux/replay, événements, dashboard, captures continues). La livraison T0 n'attend ni capteur réel ni application ni intégration complète du harness. Nevil conserve toutes les métriques finales. Prochain bloc proposé à Icham : C1 à C3 jusqu'à T0 livrable ; ce tour ne lance aucune implémentation.

**Consigne de livraison Nevil transmise par Icham : prédictions seules, pas de métriques finales calculées par l'agent ML.** Contrat relu à `08562e3eee29a9302ed2dfba2a4112b251770051`. T0 prioritaire : `metadata.json` + `predictions.csv`, exactement `clip_id,probability_leak`, couverture des 402 clips validation/test du v2 gelé ; aucun seuil appliqué ni colonne de métadonnée dataset. Après gel du checkpoint, T1/T2/T3 optionnels en inférence seule, mêmes poids et méthode de score. Nevil possède seuils, agrégations, métriques et comparaisons finales. Les diagnostics de développement restent séparés, sans test. Manque actuel côté V0 : calcul de probabilité de classe vérifié (`score_type=none`). Aucune prédiction ni métrique nouvelle produite dans ce tour. [[Journal Icham#Consigne de livraison T0 et stress — Nevil]].
