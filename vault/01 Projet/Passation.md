...
**Matériel envisagé par Icham : quatre H100 sur quatre machines séparées.** Cette topologie convient à plusieurs fits indépendants, notamment les trois folds d'une future campagne ; pas de synchronisation de gradients entre ces jobs. Accès nouveaux non fournis/vérifiés, aucune installation ni campagne distribuée lancée. Même code/environnement/poids/données et dossiers de runs isolés à vérifier avant usage ; ne pas augmenter le nombre d'essais pour remplir les GPU. Un seul fit distribué resterait une adaptation distincte dépendante du réseau. Comparaison H200 dans [[Journal Icham]], aucun gain mesuré sur PIPE ni migration décidée.
