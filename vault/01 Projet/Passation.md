...
**Deux H100, logiciels et socle train vérifiés sur les deux machines.** `hicham@89.169.123.193` exécute D3 ; `ich@195.242.28.46` possède `/home/ich/pipe-v0/.venv-repro` reconstruit du lock existant sur `code-a18b22e` : 132 paquets, imports, calcul/backward CUDA et 214 tests sans skip. Transfert646fichiers terminé et vérifié : baseQwen43fichiers,598WAVtrain et5caches/provenances, aucun fichier val/test/externe. Preuves `docs/evidence/runtime-h100-2-001/`, vérificateur `e142f66`. Aucun chargement/forward Qwen ni fit métier encore exécuté sur la seconde : établir cette parité avant des fits répartis justifiés. D3 reste sur la première ; aucun essai ajouté ni mémoire mutualisée.
...
**Demande actuelle d'Icham : tester sur des données non vues.** Extension séparée en inférence seule, code `141ff92`, sur les98 réservés du fold0 D0, groupes disjoints des32. Préinscription `78e3b455…`, dossier `/home/hicham/pipe-v0/artifacts/d3-heldout-98-141ff92-001`. Même checkpoint/score, seuil diagnostique0,5 non réglé, 98 observations/196 forwards maximum. Publication du protocole avant observation ; aucun résultat réservé encore à ce jalon. Pas de nouveau fit ni LoRA, val/test officiels et externe fermés. Comparaison A0 descriptive uniquement : A0 avait appris500 clips, D3 seulement32.
