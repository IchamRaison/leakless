...
**Moyens actuels rappelés par Icham : deux H100, sur deux machines.** `hicham@89.169.123.193` possède le runtime et les données du projet ; `ich@195.242.28.46`, hôte `computeinstance-e00fgs9mdc3y0zkpk7`, a une H100 80GB HBM3 (81 559 Mio), pilote 580.173.02 et environ 1,2 To libres. Second inventaire en lecture seule : Python 3.12.3, git/tar/scp et module venv présents, mais `pipe-v0`, `.venv-repro`, pip/ensurepip et uv absents ; aucun environnement installé par l'agent. Deux accès vérifiés, pas deux environnements prêts. Préparer le second avant d'y répartir des fits indépendants déjà justifiés ; aucun essai ajouté pour remplir les GPU ni mémoire automatiquement mutualisée.
...
**D3 en préparation :** un seul essai A neuf sur les 32 témoins D0, budget maximal 1 000 pas, loss/scoring/poids Qwen BF16 inchangés ; arrêt programmé seulement si NLL binaire <0,1 et 32/32 décisions correctes. Protocole dans [[Diagnostic causal - exécution#D3 — mémorisation du chemin Qwen, protocole avant fit]]. Implémentation confiée sur deux nouveaux fichiers ; tests et préinscription machine à faire, aucun fit D3. Une réussite ne validerait pas la généralisation ; un échec ne localiserait pas seul la cause.
