...
**V2 — 13 septembre : reprise active ; objectif final TSLM publié dans le vault.** [[Plan V2 - fiabilité et parité des scores#7. Objectif final — démontrer la valeur du TSLM]] : comprendre/investiguer les événements, comparer gabarit, Qwen sur mesures et TSLM sur séries ; aucune capacité présumée acquise. [[V2 ML - exécution]] : diagnostic 598 train terminé (sonde TimeNet AUC groupée moyenne 0,636 contre C1 0,938, pas un score TSLM), A/C retenues et B omise. **A et C + rechargements neufs PASS, écarts nuls**, reçus SHA `ed2bfdc5…` / `50ec36e9…` à `04d53b6` ; séquence `98052` terminée normalement, ne pas redémarrer. Runner et évaluateur externe publiés `b750f5d`, 144 tests runtime sans skip. Exporteur codé/testé localement, assemblage runtime final à vérifier. Aucun fit TSLM V2 ni score externe ; manifeste externe figé SHA `84007801…`. Prochaine action : vérifier cet assemblage, préinscrire puis lancer les six fits train comparatifs, sans sélection sur validation/test. **Checkout actif `/home/animus/ehl-hackathon-zurich-v2-recovery`** ; ancien Git endommagé conservé, ne pas y commiter. Le benchmark final à trois approches reste distinct de cette campagne sur extraits.
