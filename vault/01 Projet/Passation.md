...
**Publication rapide demandée :** README de `feat/c1-opentslm-qwen` réécrit pour les évaluateurs : accès/tunnel, replay complet H100 et contrat. Pas de nouveau packaging/installation ; code, preuves et notes publiés ensemble. Dépôt code privé désormais `IchamRaison/leakless` : transmettre la branche et fournir les accès GitHub/SSH autorisés. Clone seul non autonome, poids déjà sur H100. [[C1 OpenTSLM Qwen - exécution]].

**Livraison actuelle : [[C1 OpenTSLM Qwen - exécution]].** C1 → historique64×10 → OpenTSLM-SP + Qwen3.5-4B/LoRA → description et proposition Nevil après31secondes hautes. Branche `feat/c1-opentslm-qwen`, code/preuves `2318dee` ; service `77c174f`, PID129483 au lancement, première H100, loopback8020.39tests et recetteHTTP43s réussis, durées déterministes, file SQLite durable, zéro envoi externe. Deux fits terminés, ne pas relancer : premier3/25, LoRA retenu16/25 sur chronologies artificielles dev,9fallbacks,8/25 inversions, reload exact. Dans la recette, persistance reconnue mais fin corrigée par gabarit explicite. Aucun gain terrain ou supériorité TSLM démontré. Prochaine action : raccorder backend/Monitor avec `docs/TEMPORAL_LANGUAGE_ENDPOINT.md` et tunnel8020 ; checkpoint/SHA/reprise documentés dans la note. Ancien8019/repli195 préservés. La livraison ci-dessous est la version C1 antérieure.

**Livraison actuelle : [[C1 temporel - exécution]].** C1 + suivi SQLite causal + aperçus fonctionne par endpoint sur la première H100, `127.0.0.1:8019` via tunnel SSH. Service `5553e14`, PID123337 au lancement ; code/preuves publiés `8baecab`, branche `feat/c1-temporal-alerts`. Recette HTTP réelle12fenêtres/1Hz : mêmes neuf mesures/scores qu'offline, un événement et deux aperçus sans envoi, p95serveur11,06ms sur ce court essai. LSTM entraîné et évalué, **non promu** (AUC groupe validation0,375/confirmation0,590278), contrôle sans ordre meilleur. Aghashahi réaffecté avec protocole publié avant fit ; aucun réglage après confirmation. La qualité terrain de C1/persistance reste non validée. Contrat/tunnel : `docs/TEMPORAL_ENDPOINT.md`. Prochaine action côté intégration : raccorder le Monitor à ces routes ; pas de frontend modifié ni accès au placeholder `195.242.28.46`. Aucun test/envoi WhatsApp. Ne pas relancer ces fits ni réutiliser la confirmation observée pour sélectionner une recette.

**Historique du plan initial :** [[Plan C1 - suivi temporel et notifications]] sépare M1 et M2. L'autorisation d'implémenter/réaffecter Aghashahi est ensuite arrivée ; l'état réel ci-dessus remplace l'ancien jalon «planification uniquement». Les séquences n'apportent toujours aucune annotation fiable de début/fin de fuite.

**Précision d'Icham : `195.242.28.46` porte un endpoint placeholder pour installer la V1 en repli.** Préserver le service ; ne pas y relancer des fits sur la foi de l'ancien relevé GPU. URL complète/route/contrat/accès non fournis, V1 effective non vérifiée ; aucun contrôle SSH ou appel de cet endpoint dans cette planification. Prochaine action : faire valider le plan et les rôles données/interface/notification avant exécution. Le goal antérieur n'est ni terminé ni remplacé automatiquement par cette demande de plan.

**[[Rush Qwen 27B - exécution]] : calculs déjà lancés terminés.** Trois fits internes MB4/1époque (63/47/41 pas, 500/373/323 présentations), puis trois évaluations en processus neufs sur 98/225/275 clips réservés du train. Base gelée, scoring et budget inchangés ; aucun test officiel/externe. Le contrôle du wrapper `Predictor.score/predict` sur train passe, sans intégration applicative revendiquée. Au dernier contrôle SSH, aucun job de campagne restant et les deux GPU à 0 Mio/0 %. Audit indépendant du fold0 seulement terminé ; audit consolidé, rapatriement des dernières preuves et bilan final encore inachevés. Ne pas relancer les fits. Code immuable `dae7e22`, gate `b70a408`, dossiers distants `rush-qwen27b-001/fold-N`, preuves `docs/evidence/tslm-v2/qwen27b-rush-001/`. Cette campagne ne termine pas le plan produit en dix étapes ci-dessous.

**Deux H100, logiciels et socle train vérifiés sur les deux machines.** `hicham@89.169.123.193` a exécuté D3 puis son évaluation réservée, tous deux terminés ; `ich@195.242.28.46` possède `/home/ich/pipe-v0/.venv-repro` reconstruit du lock existant sur `code-a18b22e` : 132 paquets, imports, calcul/backward CUDA et 214 tests sans skip. Transfert646fichiers terminé et vérifié : baseQwen43fichiers,598WAVtrain et5caches/provenances, aucun fichier val/test/externe. Preuves `docs/evidence/runtime-h100-2-001/`, vérificateur `e142f66`. Aucun chargement/forward Qwen ni fit métier encore exécuté sur la seconde : établir cette parité avant des fits répartis justifiés. Aucun apprentissage distribué ni mémoire mutualisée.

`iche@89.169.97.196` répond en SSH mais refuse la clé explicite puis les identités normales du poste (`Permission denied (publickey)`). Les précédents `icham@89.169.102.78` et `ich@89.169.122.233` avaient également refusé l'authentification ; pas retestés ici. Vérifier comptes et autorisation de clé pour les accès refusés ; ne pas supposer que les nouvelles adresses correspondent à des machines supplémentaires plutôt qu'à des remplacements. Preuves : [[Journal Icham#13 septembre — nouveaux essais SSH, une H100 supplémentaire accessible]].

La topologie convient à plusieurs fits indépendants, notamment les trois folds d'une future campagne ; pas de synchronisation de gradients entre ces jobs. Même code/environnement/poids/données et dossiers de runs isolés à vérifier avant usage ; ne pas augmenter le nombre d'essais pour remplir les GPU. Un seul fit distribué resterait une adaptation distincte dépendante du réseau. Aucune campagne distribuée lancée ; ne pas réécrire les chemins absolus des anciennes préinscriptions pour les déplacer. Comparaison H200 dans [[Journal Icham]], aucun gain mesuré sur PIPE ni migration décidée.

**D2 terminé et vérifié indépendamment :** trois fits HGB/200 itérations, artefacts `160655a`, reçu `48ac9de1…`, sans Qwen ni nouvelle donnée. AUC groupe réservée moyenne 0,854167 contre 0,636218 pour la sonde linéaire et 0,937500 pour C1 fixe. Trois gains face à la sonde linéaire, trois retards face à C1 ; AUC fit 1,0 dans chaque fold. L'entrée conserve une information exploitable, sans preuve que Qwen sait l'apprendre. [[Diagnostic causal - exécution#D2 — résultats vérifiés et portée]]. Code `303609e`, 214 tests runtime, préinscription `67ed6bd5…` publiée `3505928` avant fit ; ne pas relancer D2.

**D3 terminé normalement au pas 400 ; reload neuf réussi, écart maximal nul.** Un seul fit sur 32 clips équilibrés, 100 époques /3 200 présentations : 32/32 corrects, NLL binaire 0,000826279, Qwen gelé inchangé et loss complète d'origine. Reçu train `5e782e54…`, reload `3fe96a1a…`, checkpoint `3e99f7ae…`. Ce résultat prouve la mémorisation de ces exemples, pas leur généralisation. Handle20214 terminé ; ne pas relancer le fit. Preuves `docs/evidence/tslm-v2/causal-d3-001/final/` et [[Diagnostic causal - exécution#D3 — résultat final et extension sur groupes réservés]].

**Évaluation demandée par Icham terminée et vérifiée :** checkpoint D3 fixe sur les98 réservés du fold0D0, groupes disjoints des32. **67/98corrects,21fuites manquées sur73,10fausses alertes sur25non-fuite/bruits**, au seuil0,5 non réglé. AUC clip0,741370 /groupe0,798077 ; NLL binaire1,419433. Code `141ff92`, préinscription `78e3b455…` publiée `84c1890` avant observation, reçu `17f8af51…` ; preuves `docs/evidence/tslm-v2/d3-heldout-98-001/`.98observations/196forwards, zéro apprentissage, poids inchangés ; scores/NLL/métriques revérifiés localement. Aucun test officiel/externe. A0 sur les mêmes98 : AUC0,710/0,760, comparaison seulement descriptive (500clips appris contre32). Prochaine étape : décider un contraste de généralisation à budget comparable, pas relancer la mémorisation ni adopter LoRA automatiquement. La qualité reste insuffisante ; ces98ne contiennent que2vrais sans-fuite et23bruits.
