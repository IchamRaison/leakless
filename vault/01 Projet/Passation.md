...
**V2 — 13 septembre : reprise active ; objectif final TSLM publié dans le vault.** [[Plan V2 - fiabilité et parité des scores#7. Objectif final — démontrer la valeur du TSLM]] : comprendre/investiguer les événements, comparer gabarit, Qwen sur mesures et TSLM sur séries ; aucune capacité présumée acquise. [[V2 ML - exécution]] : diagnostic 598 train terminé (sonde TimeNet AUC groupée moyenne 0,636 contre C1 0,938, pas un score TSLM), A/C retenues et B omise. Code A/C `04d53b6` publié, 117 tests runtime sans skip et nouveau cache806 exactement vérifié. **A + reload PASS, delta zéro ; C + reload en cours dans la même séquence `98052`**, aucun entraînement V2 tant que les deux gates ne sont pas complets. Runner de campagne et mode d'évaluation externe codés/testés localement, tests runtime combinés à faire. Manifeste externe figé SHA `84007801…`, aucun score externe. Prochaine action : suivre C sans redémarrage, vérifier l'orchestration, puis préinscrire et lancer les fits bornés après PASS. **Checkout actif `/home/animus/ehl-hackathon-zurich-v2-recovery`** ; ancien Git endommagé conservé, ne pas y commiter. Le benchmark final à trois approches reste distinct de cette campagne sur extraits.
