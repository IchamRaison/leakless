...
**Matériel envisagé par Icham : quatre H100 sur quatre machines séparées.** Deux nouveaux accès fournis et testés : `icham@89.169.102.78` et `ich@89.169.122.233`. Les deux serveurs répondent en SSH, mais refusent l'authentification (`Permission denied (publickey)`), y compris avec la clé explicitement sélectionnée qui fonctionne sur `hicham@89.169.123.193`. Accès existant revérifié ; GPU/runtime des nouvelles machines non vérifiés, quatrième accès non fourni. Vérifier les comptes et l'autorisation de la clé publique avant inventaire. Commandes et résultats : [[Journal Icham#13 septembre — vérification des deux nouveaux accès SSH]].
