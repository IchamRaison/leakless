...
| **Les prédictions de C2b sont sensibles aux perturbations d'ordre temporel et de phase** | même définition figée, réajustée de façon déterministe sur T0/train, jamais ajustée sur les données stressées (empreinte de modèle identique dans les quatre runs). Métriques indépendantes du seuil, test : corrélation des probabilités avec T0 de **0,729** (T2) et **0,705** (T3) ; Δ clip AUC stress − T0 **−0,097** (T2, IC95 [−0,192, −0,031]) et **−0,121** (T3, IC95 [−0,316, −0,059]), appariés | **compatible with degradation under stress** (T2, T3) ; T1 *inconclusive* | T2/T3 montrent que prédictions et discrimination changent quand l'organisation temporelle est perturbée. Ce ne sont pas des augmentations physiques démontrées comme préservant l'étiquette : **aucune pertinence physique causale n'est établie**. T1 bouge à peine, conformément à la prévision qualitative du §9 | `FINAL_EVALUATION.md` §7, `docs/C2B_HYPOTHESIS.md` §Stress |
...
| « C2b est réellement temporel » / « T2 et T3 prouvent que l'information temporelle compte » | les stress mesurent une sensibilité des prédictions à une perturbation. Ce ne sont pas des augmentations physiques démontrées comme préservant l'étiquette : ils n'établissent aucune pertinence physique causale. |
| Une conclusion de stress fondée sur le macro-F1 ou l'exactitude | le seuil d'un run stressé est recalculé sur sa validation transformée ; seules l'AUC, la corrélation des probabilités et Δp sont indépendantes du seuil. |
