...
Cette livraison est sur **`feat/c1-opentslm-qwen`**. Le dépôt est privé : demander à l'équipe l'accès GitHub et une clé SSH autorisée pour la démonstration hébergée. Aucun secret n'est inclus.

```bash
git clone --branch feat/c1-opentslm-qwen --single-branch \
