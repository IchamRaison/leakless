...
/** Live V2 readout. It never fabricates probabilities and only calls /predict on explicit user action. */
export function ModelReadout({ sample }: { sample: Sample | null }) {
  ...
  const decision = result?.abstained
    ? "ABSTAINED"
    : result?.prediction === "leak"
      ? "LEAK-ASSOCIATED"
      : result?.prediction === "no_leak"
        ? "NON-LEAK"
        : "NOT RUN";
    ...
    <div className="model-block">
      ...
      {result && (
        <div className="model-result">
          <p>{result.description ?? result.abstention_reason ?? "No model description returned."}</p>
          <small>
            Raw relative score · not calibrated · threshold {result.threshold ?? "not loaded"}
          </small>
      ...
      <p className="model-caveat">
        Live V2 is opt-in. The score is a raw model output, not a field
        probability or an inspection recommendation.
      </p>
    </div>
  );
}
